...
def main(opt):
    ...
    if opt.self_retrieval:
        pass
    else:
        data = src.data.load_data(opt.data_path)
    ...
    # index all passages
    input_paths = glob.glob(args.passage_embeddings)
    ...
    if opt.self_retrieval:
        with open('embeddings/wikipedia_embeddings_00.pkl', 'rb') as inf:
            questions_embedding = pickle.load(inf)[1]

        # with open('corpus_graph/doc_query_embeddings.pkl', 'rb') as inf:
        #     indices, questions_embedding = pickle.load(inf)
        top_ids_and_scores = index.search_knn(questions_embedding, args.n_docs)
        
        with open(f'top_ids_and_scores/{args.passage_embeddings.split("/")[2]}_0.pkl', 'wb') as outf:
            pickle.dump(top_ids_and_scores, outf)

        with open('embeddings/wikipedia_embeddings_01.pkl', 'rb') as inf:
            questions_embedding = pickle.load(inf)[1]
        top_ids_and_scores = index.search_knn(questions_embedding, args.n_docs)

        with open(f'top_ids_and_scores/{args.passage_embeddings.split("/")[2]}_1.pkl', 'wb') as outf:
            pickle.dump(top_ids_and_scores, outf)
            
        # with open('corpus_graph/doc_query_top_ids_and_scores.pkl', 'wb') as outf:
        #     pickle.dump(top_ids_and_scores, outf)
        # with open('corpus_graph/results_doc_query_dpr.tsv', 'wt') as outf:
        #     for i, idx in enumerate(tqdm(indices)):
        #         ids_and_scores = top_ids_and_scores[i]
        #         if idx not in set(ids_and_scores[0]):
        #             a += 1
        #             print('weird', i, idx)
        #         cnt = 0
        #         for e, (docid, score) in enumerate(zip(*ids_and_scores)):
        #             if idx != docid:
        #                 outf.write(f"{idx}\t{docid}\t{score}\n")
        #                 cnt += 1
        #             if cnt == 8:
        #                 break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', required=False, type=str, default=None, 
    ...
    parser.add_argument('--n-docs', type=int, default=20, help="Number of documents to retrieve per questions")
    ...
    parser.add_argument("--self_retrieval", action='store_true', help="Whether to retrieve using passage embeddings")
